...
const Clientes = () => {
  ...
  const hasNextPage = () => {
    return currentPage < totalPages && clientes.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
  };
  ...
  const getClientes = () => {
    axios
      ...
      .then((response) => {
        ...
        setClientes(sortedClientes.reverse());
        const filtered = response.data.filter((cliente) => {
          const tituloCompleto = `${cliente.telefono} ${cliente.nombre}${cliente.dni}`;
  ...
  const crearCliente = async (event) => {
    ...
    if (
      !clienteEditado.dni.trim() ||
      !clienteEditado.nombre.trim() ||
      !clienteEditado.apellidopaterno.trim() ||
      !clienteEditado.apellidomaterno.trim() ||
      !clienteEditado.sexo.trim() ||
    ...
    try {
      ...
      const nuevoCliente = {
        ...
        apellidopaterno: clienteEditado.apellidopaterno,
        apellidomaterno: clienteEditado.apellidomaterno,
        sexo: clienteEditado.sexo,
      ...
      axios
        .post(`${API_URL}/cliente`, nuevoCliente,{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const actualizarCliente = async (event) => {
    ...
    try {
      ...
      // Realizar la solicitud PUT para actualizar el Clientes
      ...
      // Actualizar la lista de Clientess
      getClientes();
      closeModal()
      setClienteEditado({
        id: null,
  ...
  const contenidoModal = (
    ...
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{clienteEditado.id ? 'Editar Cliente' : 'Crear Cliente'}</h2>
          ...
          <div class="flex-none w-full max-w-full px-3">
            <div class=" flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
              <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                <h6 class="dark:text-white font-bold">Lista De Cliente</h6>
              </div>
